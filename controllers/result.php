...
class Result extends Controller {
    ...
    function index($name = null){
        $sch = $this->model->find_school($name);
        $this->view->sch = $sch;
        //$this->view->title = ucfirst($sch->school_name);
        $this->view->title = "Results - School Portal";
        $this->view->sessions = $this->model->started_sessions($sch->school_id);
        //$this->view->terms = $this->model->started_terms($sch->school_id);
        $this->view->classes = $this->model->started_classes($sch->school_id);

        $this->view->js = array('result/js/school.js');
        $this->view->generalJS = array('ajax.js');
        $this->view->render('result/index','none');
    ...
    function design(){
        $this->view->render('result/design','none');
    }
    ...
    function check(){
        ...
        if (Input::exists()) {
			...
			if ($validation->passed() && $pin = $this->model->fake_check_pin()) {
					if($pin != 'stale'){
                    $this->view->title = "Statement of Result - School Portal";
                    list($this->view->name,$this->view->result,$this->view->folder) = $pin;
                    $this->reMapRouteToModel('teacher');
                    $this->view->resumption = $this->teacher->resumption();
                    $this->view->school_fees = $this->teacher->school_fees();
