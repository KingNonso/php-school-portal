...
class Upload
{
    ...
    public static function convertToBytes($str){
        $last = strtolower($str[strlen($str)-1]);

        if(is_numeric($last)) {
            $val = (int) $str;
        } else {
            $val = (int) substr(trim($str), 0, -1);
        }
        switch($last) {
    ...
    public function upload($renameDuplicates = true){
        ...
        if(is_array($uploaded['name'])){ //multiple file uploads
    ...
    protected function checkSize($file){
        if(!$file['tmp_name']){
            $this->messages[] = 'Please browse for and select your upload file.';
            return false;
        }elseif($file['size'] == 0){
            $this->messages[] = $file['name']. ' is empty.';
            return false;
        }elseif ($file['size'] > $this->maxSize){
            $this->messages[] = $file['name']. ' exceeds the maximum size for a file of this type (' .self::convertFromBytes($this->maxSize) .').';
            return false;
        }else{
            return true;
        }
    ...
    protected function moveFile($file){
        ...
        if($success){
            ...
            if(!is_null($this->newName)){
                $result .= ', and was renamed as '.$this->newName;
            }
            $result .= '.';
