<?php
    class User{
        ...
        public function get_profile_pic($member_id){
            return $this->_db->fetch_last('member_status','status_id','person_id',$member_id);
        }
        ...
        public function get_liked($user, $pattern = 'post_id', $id){
            ...
            if($total_like_count == 1){
                if($user_likes_it){
                    $no_of_likes = '<span class="badge"  id="like'.$id.'">1</span> You';
                }else{
                ...
                if($user_likes_it){
                    $no_of_likes =  'You + <span class="badge"  id="like'.$id.'">'.($total_like_count -1). '</span>';
                }else{
                    $no_of_likes =  '<span class="badge"  id="like'.$id.'">'.($total_like_count ). '</span> Like';
                }
        ...
        public function refToRecord($ref, $table = 'info_personal'){
        ...
        public function find_appointment($role_id){
            return $this->_db->fetch_exact('national_exco','person_id',$role_id);
        }
        ...
        public function search_box($search_term,$search_in){
            $search_term = "%".$search_term."%";
                ...
                case 'members': //search in portfolio table
                    $view =  $this->_db->get_triple_either('info_personal',array('firstname','LIKE',$search_term,'surname','LIKE',$search_term,'othername','LIKE',$search_term))->results_assoc();
                    $this->_student_count = $this->_db->get_triple_either('info_personal',array('firstname','LIKE',$search_term,'surname','LIKE',$search_term,'othername','LIKE',$search_term))->count();
                    break;
                case 'category':
                    ...
                    break;
                case 'staff':
                    $view =  $this->_db->get_triple_either('info_staff',array('firstname','LIKE',$search_term,'surname','LIKE',$search_term,'othername','LIKE',$search_term))->results_assoc();
                    $this->_student_count = $this->_db->get_triple_either('info_staff',array('firstname','LIKE',$search_term,'surname','LIKE',$search_term,'othername','LIKE',$search_term))->count();
                    break;
        ...
        public function get_team_member($member_id){
            return $this->_db->get_assoc('team', array('person_id','=',$member_id))->results_assoc();
        }
        public function get_my_forum($user){
            return $this->_db->get_assoc('forums_members', array('person_id','=',$user))->results_assoc();
        }
        public function get_forums_listed($user){
            $list = array();

            $forums = $this->_db->get_assoc('forums_members', array('person_id','=',$user))->results_assoc();
        ...
        public function all_people(){
            return $this->_db->getAll_assoc('info_personal')->results_assoc();

        }
        public function get_multiple_forum($user, $forum){
        return $this->_db->get_multiple('forums_members', array('person_id','=',$user,'forum_id','=',$forum))->results_assoc();
    }
